...
cv::Mat flatten(cv::Mat image) {
    ...
    cv::Mat col_avg;
    cv::reduce(image, col_avg, 0, cv::REDUCE_AVG, CV_32F);
    float* avg_ptr = col_avg.ptr<float>(0);

    // Step 2: Scale vector so max element is 1.0f, and precompute inverse factors
    double min_val, max_val;
    cv::minMaxLoc(col_avg, &min_val, &max_val);

    std::vector<float> inv_scales(W, 1.0f);
    std::vector<float> W_elements(W, 0.0f); // Store for debugging

    if (max_val > 0.0) {
        for (int x = 0; x < W; ++x) {
            W_elements[x] = avg_ptr[x] / static_cast<float>(max_val); // Calculate scaled W
            if (avg_ptr[x] > 1e-6f) { 
                inv_scales[x] = static_cast<float>(max_val) / avg_ptr[x];
            } else {
                inv_scales[x] = 1.0f;
            }
        }
    ...
    if (outfile.is_open()) {
        ...
        std::cerr << "Warning: Could not open res.txt for writing.\n";
    }
