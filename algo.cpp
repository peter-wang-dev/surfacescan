...
extern "C"
{
		...
		bool* isOverLoad,bool* isHazeOverload)
	{
		...
		cv::Mat defectmap=dehazed.clone(); // For demonstration, subtract the haze image from the ring image to get defect map
	...
	{ 
		...
		std::println("Identified {} defects in channel {}",defects.size(),static_cast<int>(channelID));
		...
		write_log(LogType::Info,"EndChannelProcess","Saving to file");
		if(dir!="")
		{
			cv::Mat defect_annotation=drawmap(fullimage,defects);
...
{ 
	...
	for(size_t k=1;k<Intensities.size()&&k<DSizes.size();k++)
	{
        ...
        cv::Mat binary = (fullmap >= intensity); // yields 8-bit mask (0 or 255) in OpenCV expression context on most builds
        // Ensure 8-bit 0/255
		...
		std::println("Intensity threshold: {}, DSize: {}, Found {} connected components",intensity,dsize,nLabels);
...
{
	cv::Mat defectmap;
	cv::cvtColor(fullmap,defectmap,cv::COLOR_GRAY2BGR);
	int numdraw=0;
	for(const auto& defect:defects)
	{
		if(numdraw++>10) break; // limit to 100 defects for drawing
		...
		cv::Size axes(static_cast<int>(defect.XSize/2),static_cast<int>(defect.YSize/2));
		cv::ellipse(defectmap,center,axes,0,0,360,cv::Scalar(0,0,255),2); // red ellipse
		...
		cv::putText(defectmap, text, textOrg, fontFace, fontScale, cv::Scalar(0,0,0), thickness+2, cv::LINE_AA);
		cv::putText(defectmap, text, textOrg, fontFace, fontScale, cv::Scalar(255,255,255), thickness, cv::LINE_AA); 
	}
	return defectmap;
}
